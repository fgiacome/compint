...
def valid(state: State):
    ...
    if x_wins and o_wins:
        clash = True
    return (
        len(state.x.intersection(state.o)) == 0
        and len(state.o) - len(state.x) in {0, 1}
        and clash == False
    )
...
while delta > epsilon:
    ...
    for state, value in state_values.items():
        ...
        if win(state) != 0 or len(state.free) == 0:
            continue
        ...
        next_value = max(
            [
                ...
                sum(
                    [
                        state_values[State(successor.x, successor.o.union({cell}))]
                        for cell in successor.free
                    ]
                )
                / len(successor.free)
...
while win(state) == 0:
    move = min(
        sample([move for move in state.free], k=len(state.free)),
        key=lambda m: state_values[State(state.x.union({m}), state.o).swap()],
    )
